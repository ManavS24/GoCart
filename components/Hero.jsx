...
const Hero = () => {
    ...
    // Read off the catalogue rather than hardcoded, so the claim cannot drift
    // from what the shop actually sells.
            ...
            <div className='flex max-xl:flex-col gap-8 max-w-7xl mx-auto my-10'>
                <div className='relative flex-1 flex flex-col bg-green-200 rounded-3xl xl:min-h-100 group'>
                    <div className='p-5 sm:p-16'>
                        <div className='inline-flex items-center gap-3 bg-green-300 text-green-600 pr-4 p-1 rounded-full text-xs sm:text-sm'>
                            <span className='bg-green-600 px-3 py-1 max-sm:ml-1 rounded-full text-white text-xs'>NEW</span> 20% off your first order with code NEW20 <ChevronRightIcon className='group-hover:ml-2 transition-all' size={16} />
                        </div>
                        ...
                        <div className='text-slate-800 text-sm font-medium mt-4 sm:mt-8'>
                            <p>Starts from</p>
                            <p className='text-3xl'>{lowestPrice === null ? '—' : `${currency}${formatAmount(lowestPrice)}`}</p>
                        </div>
                    ...
                    <Image className='sm:absolute bottom-0 right-0 md:right-10 w-full sm:max-w-sm' src={assets.hero_model_img} alt="" aria-hidden="true" />
                </div>
                <div className='flex flex-col md:flex-row xl:flex-col gap-5 w-full xl:max-w-sm text-sm text-slate-600'>
                    <Link href='/shop' className='flex-1 flex items-center justify-between w-full bg-orange-200 rounded-3xl p-6 px-8 group'>
                        ...
                        <Image className='w-35' src={assets.hero_product_img1} alt="" aria-hidden="true" />
                    </Link>
                    <Link href='/pricing' className='flex-1 flex items-center justify-between w-full bg-blue-200 rounded-3xl p-6 px-8 group'>
                        <div>
                            <p className='text-3xl font-medium bg-gradient-to-r from-slate-800 to-[#78B2FF] bg-clip-text text-transparent max-w-40'>Plus members</p>
                            <p className='flex items-center gap-1 mt-4'>View more <ArrowRightIcon className='group-hover:ml-2 transition-all' size={18} /> </p>
                        </div>
                        <Image className='w-35' src={assets.hero_product_img2} alt="" aria-hidden="true" />
                    </Link>
                </div>
            </div>
            <CategoriesMarquee />
